...
@pytest.fixture
def eliminator() -> GaussianEliminationSolver:
    A = [[2, -6, -1, -38], [-3, -1, 7, -34], [-8, 1, -2, -20]]
    eliminator = GaussianEliminationSolver()
    eliminator.set(A)
    return eliminator
...
def test_solve(eliminator):
    ...
    assert_allclose(result, [4, 8.0, -2.0], rtol=1e-6)
...
def test_eliminate(eliminator):
    ...
    assert_allclose(
        eliminator.A,
        [[2.0, -6.0, -1.0, -38.0], [0.0, -10.0, 5.5, -91.0], [0.0, 0.0, -18.65, 37.3]],
        rtol=1e-6,
    )
